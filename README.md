...
| **Modèle** | `meteofrance_seamless`, fixé explicitement plutôt que le `best_match` par défaut d'Open-Meteo. `best_match` s'est révélé être un mélange opaque entre fournisseurs (AROME, ICON, GFS, ECMWF selon le point), avec des sauts de modèle imprévisibles d'un appel à l'autre — mesuré jusqu'à 30° d'écart de direction sur un même spot entre deux requêtes. La chaîne Météo-France est stable et choisit automatiquement la maille la plus fine disponible : AROME HD (~1,3 km) sur la France métropolitaine, AROME (~2,5 km) en périphérie, ARPEGE (~10 km) au large et hors de France (Portugal, Espagne). C'est aussi le modèle de référence que la plupart des riders français consultent sur Windguru — les écarts qui restent viennent d'un choix de modèle différent côté Windguru (souvent GFS par défaut), pas d'une donnée fausse : deux modèles météo légitimes divergent normalement sur le vent côtier fin, surtout sur les spots à effet de site marqué (Tarifa, Almanarre) qu'aucun modèle à cette résolution ne capture parfaitement. |
